...
Script.prototype.writeChunk = function (chunk) {
  assert(Buffer.isBuffer(chunk) || typeof chunk === 'number', 'Expected Buffer or Number');
...
Script.createMultisigOutput = function (m, pubkeys) {
  ...
  assert(m, 'Missing number of required signatures');
  assert(m <= pubkeys.length, 'Number of required signatures cannot be higher than the number of public keys');

  var n = pubkeys.length;
  var NUM_OP = opcodes.OP_1 - 1;
  var script = new Script();
  script.writeChunk(NUM_OP + m);
  pubkeys.forEach(function (pubkey) {
    script.writeChunk(pubkey);
  });
  script.writeChunk(NUM_OP + n);
...
Script.createMultisigInput = function (signatures) {
  var script = new Script();
  script.writeChunk(opcodes.OP_0);
  signatures.map(function (signature) {
    script.writeChunk(signature);
  });
  script.updateBuffer();
  return script;
};
